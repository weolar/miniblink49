...
public:
    ...
    static void showOpenOrSaveDialogEnd(ShowOpenOrSaveDialogThreadInfo* info)
    {
        ...
        if (0 != info->paths->GetSize())
            result = gin::Converter<base::ListValue>::ToV8(info->isolate, *info->paths);
        ...
        info->callback.Get(info->isolate)->Call(recv, 2, argv);
    ...
    static DWORD __stdcall showOpenOrSaveDialogThreadEntryPoint(void* param)
    {
        ...
        info->canceled = !showOpenOrSaveDialog(info->isOpenOrSave, info->hwnd, *info->title, *info->buttonLabel, *info->defaultPath, *info->filters, info->fileDialogProperty, info->paths);
        if (info->sync) {
            *info->sync = 1;
            return 0;
        }
        ThreadCall::callUiThreadAsync([info] {
            showOpenOrSaveDialogEnd(info);
        });
        return 0;
    }
    ...
    void _showOpenOrSaveDialogApi(bool isOpenOrSave, bool isSync, const v8::FunctionCallbackInfo<v8::Value>& args)
    {
        ...
        if (0 != info->paths->GetSize())
            result = gin::Converter<base::ListValue>::ToV8(isolate, *info->paths);
...
//         } else
    ...
    struct ShowMessageBoxThreadInfo {
        ...
        int buttonsSize;
        v8::Isolate* isolate;
        gin::Promise<gin::Dictionary>* promise;
        HWND hwnd;
        int result;
    };
    ...
    static DWORD __stdcall showMessageBoxThreadEntryPoint(void* param)
    {
        ...
        int result = ::MessageBoxW(nullptr, info->message.c_str(), info->title.c_str(), info->uType);
        ...
        if (info->promise) {
            ThreadCall::callUiThreadAsync([info] {
                ...
                gin::Dictionary dict = gin::Dictionary::CreateEmpty(info->isolate);
    ...
    void showMessageBoxSyncImpl(const v8::FunctionCallbackInfo<v8::Value>& args, bool isSync)
    {
        ...
        info->message = base::UTF8ToWide(message);
        info->result = -1;
        info->self = this;
        info->uType = uType;
        info->title = base::UTF8ToWide(title);
        ...
        if (!isSync) {
            info->promise = new gin::Promise<gin::Dictionary>(isolate);
...
private:
    ...
    static void pushStringToVector(std::vector<wchar_t>* buffer, const std::wstring& str)
    {
    ...
    static bool showOpenOrSaveDialog(
        bool isOpenOrSave,
        HWND parentWindow,
    ...
    static void splitPathFromGetOpenFileNameResult(const std::vector<wchar_t>& fileResult, base::ListValue* paths)
    {
    ...
    static unsigned selectDir(HWND parentWindow,
