...
// 静态方法，用于注册类和方法
void Electron::init(Local<Object> target, Environment* env) {
...
// new方法
void Electron::newFunction(const v8::FunctionCallbackInfo<v8::Value>& args) {
...
// 空实现
void Electron::nullFunction(const v8::FunctionCallbackInfo<v8::Value>& args) {
}

Persistent<Function> Electron::constructor;
...
static void initializeAppApi(Local<Object> target, Local<Value> unused, Local<Context> context, const NodeNative* native) {
    ...
    Electron::init(target, env);
}

NODE_MODULE_CONTEXT_AWARE_BUILTIN_SCRIPT_MANUAL(atom_browser_app, initializeElectronApi, &nativeHello)
