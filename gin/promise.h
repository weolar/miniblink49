...
//         //gin_helper::Locker locker(isolate());
        ...
        gin::MicrotasksScope microtasks_scope(isolate());
        v8::Context::Scope context_scope(GetContext());

        return GetInner()->Resolve(GetContext(), gin::ConvertToV8(isolate(), value));
    }

    template <typename... ResolveType>
    v8::MaybeLocal<v8::Promise> Then(std::function<void(ResolveType...)>&& cb)
    {
...
template <typename T>
struct Converter<gin::Promise<T>> {
    static v8::Local<v8::Value> ToV8(v8::Isolate* isolate, const gin::Promise<T>& val)
    {
