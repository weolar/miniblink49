...
typedef int (__stdcall* FN_WSAIoctl)(
  SOCKET s, 
...
static void win_update_buffer_size(curl_socket_t sockfd)
{
  ...
  static FN_WSAIoctl s_WSAIoctl = NULL;
  if (!s_WSAIoctl)
    s_WSAIoctl = (FN_WSAIoctl)GetProcAddress(GetModuleHandleW(L"ws2_32.dll"), "WSAIoctl");
  if (!s_WSAIoctl)
    return;

  result = s_WSAIoctl(sockfd, SIO_IDEAL_SEND_BACKLOG_QUERY, 0, 0,
