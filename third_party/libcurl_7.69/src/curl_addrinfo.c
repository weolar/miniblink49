...
typedef INT(__stdcall* FN_getaddrinfo)(
  PCSTR           pNodeName,
  PCSTR           pServiceName,
  const void *pHints,
  void      *ppResult
...
{
  static FN_getaddrinfo s_getaddrinfo = NULL;
  if (s_getaddrinfo)
    return s_getaddrinfo(pNodeName, pServiceName, pHints, ppResult);
  s_getaddrinfo = (FN_getaddrinfo)GetProcAddress(GetModuleHandleW(L"ws2_32.dll"), "getaddrinfo");
  if (!s_getaddrinfo)
    return (WSABASEERR + 1003);
  return s_getaddrinfo(pNodeName, pServiceName, pHints, ppResult);
}

typedef void(__stdcall* FN_freeaddrinfo)(void* pAddrInfo);
void freeaddrinfo_xp(void* pAddrInfo)
{
  static FN_freeaddrinfo s_freeaddrinfo = NULL;
  if (s_freeaddrinfo) {
    s_freeaddrinfo(pAddrInfo);
    return;
  }
  s_freeaddrinfo = (FN_freeaddrinfo)GetProcAddress(GetModuleHandleW(L"ws2_32.dll"), "freeaddrinfo");
  if (!s_freeaddrinfo)
    return;
  s_freeaddrinfo(pAddrInfo);
}
                    ...
                    Curl_addrinfo **result)
{
  ...
  *result = NULL; /* assume failure */
  ...
  error = getaddrinfo_xp(nodename, servname, hints, &aihead);
  if(error)
    return error;
...
#else
    /* rfc3493 obsoleted */
    error = (WSABASEERR + 1001)/*EAI_NODATA*/;
#endif
#ifdef USE_WINSOCK
    SET_SOCKERRNO(error);
#endif
...
#else
  int res = (getaddrinfo_xp)(hostname, service, hints, result);
#endif
  if(0 == res)
    /* success */
