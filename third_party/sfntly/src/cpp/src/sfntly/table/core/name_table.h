...
class NameTable : public SubTableContainerTable, public RefCounted<NameTable> {
 public:
  ...
  // Note: ICU UChar* convention requires caller to delete[] it.
  virtual wchar_t* Name(int32_t index);
  ...
  // returned.
  // Note: ICU UChar* convention requires caller to delete[] it.
  virtual wchar_t* Name(int32_t platform_id, int32_t encoding_id,
                      int32_t language_id, int32_t name_id);
 ...
 private:
  ...
  static void ConvertToNameBytes(const wchar_t* name, int32_t platform_id,
                                 int32_t encoding_id, ByteVector* b);

  // Note: ICU UChar* convention requires caller to delete[] it.
  static wchar_t* ConvertFromNameBytes(ByteVector* name_bytes,
