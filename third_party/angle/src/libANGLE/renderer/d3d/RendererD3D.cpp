...
gl::Error RendererD3D::applyTextures(const gl::Data &data)
{
    ...
    framebufferTextures.resize(gl::IMPLEMENTATION_MAX_FRAMEBUFFER_ATTACHMENTS);
...
std::string RendererD3D::getVendorString() const
{
    ...
    if (getLUID(&adapterLuid))
    {
        ...
        sprintf(adapterLuidString, /*sizeof(adapterLuidString),*/ "(adapter LUID: %08x%08x)", adapterLuid.HighPart, adapterLuid.LowPart);
...
void RendererD3D::insertEventMarker(GLsizei length, const char *marker)
{
    ...
    convertedChars = mbstowcs(wcstring.data(), marker, length);
    wcstring[length] = L'\0';
    size_t err = 0;
