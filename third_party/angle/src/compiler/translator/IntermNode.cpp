...
float __cdecl angle_asinhf(float x)
{
    return asinhf(x);
}

float __cdecl angle_acoshf(float x)
{
    return acoshf(x);
}

float __cdecl angle_atanhf(float x)
{
    return atanhf(x);
}

float __cdecl angle_roundf(float x)
{
    return roundf(x);
}

float __cdecl angle_exp2f(float x)
{
    return exp2f(x);
}
...
TConstantUnion *TIntermConstantUnion::foldUnaryWithSameReturnType(TOperator op, TInfoSink &infoSink)
{
    ...
    for (size_t i = 0; i < objectSize; i++)
    {
        switch(op)
        {
          ...
          case EOpAsinh:
            if (!foldFloatTypeUnary(operandArray[i], &angle_asinhf, infoSink, &resultArray[i]))
                return nullptr;
            break;
          ...
          case EOpAcosh:
            ...
            if (getType().getBasicType() == EbtFloat && operandArray[i].getFConst() < 1.0f)
                UndefinedConstantFoldingError(getLine(), op, getType().getBasicType(), infoSink, &resultArray[i]);
            else if (!foldFloatTypeUnary(operandArray[i], &angle_acoshf, infoSink, &resultArray[i]))
                return nullptr;
            break;
          ...
          case EOpAtanh:
            ...
            if (getType().getBasicType() == EbtFloat && fabsf(operandArray[i].getFConst()) >= 1.0f)
                UndefinedConstantFoldingError(getLine(), op, getType().getBasicType(), infoSink, &resultArray[i]);
            else if (!foldFloatTypeUnary(operandArray[i], &angle_atanhf, infoSink, &resultArray[i]))
                return nullptr;
            break;
          ...
          case EOpRound:
            if (!foldFloatTypeUnary(operandArray[i], &angle_roundf, infoSink, &resultArray[i]))
                return nullptr;
            break;
          ...
          case EOpExp2:
            if (!foldFloatTypeUnary(operandArray[i], &angle_exp2f, infoSink, &resultArray[i]))
                return nullptr;
            break;
