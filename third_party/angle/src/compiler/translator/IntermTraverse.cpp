...
void TLValueTrackingTraverser::addToFunctionMap(const TName &name, TIntermSequence *paramSequence)
{
    ...
    mFunctionMap.insert(std::pair<TName, TIntermSequence *>(name, paramSequence));
}
...
TIntermSequence *TLValueTrackingTraverser::getFunctionParameters(const TIntermAggregate *callNode)
{
    ...
    TMap<TName, TIntermSequence *, TNameComparator>::iterator it = mFunctionMap.find(callNode->getNameObj());
    if (it == mFunctionMap.end())
        return nullptr;
    TIntermSequence* result = it->second;
    return result;
}
