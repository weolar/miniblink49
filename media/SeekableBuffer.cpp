...
SeekableBuffer::SeekableBuffer(int backward_capacity, int forward_capacity)
    ...
    forward_bytes_(0),
    current_time_(base::TimeDelta::FromMicroseconds(kint64min))
{
...
{
    BufferQueue::iterator it = buffers_.begin();
...
bool SeekableBuffer::Append(DataBuffer* buffer_in)
{
    if (buffers_.empty() && buffer_in->timestamp() != base::TimeDelta::FromMicroseconds(kint64min)) {
...
{
    if (size > 0) {
        std::unique_ptr<DataBuffer> data_buffer = DataBuffer::CopyFrom(data, size);
        return Append(data_buffer.release());
    } else {
...
bool SeekableBuffer::SeekBackward(int size)
{
    ...
    UpdateCurrentTime(current_buffer_ != buffers_.end(), (*current_buffer_), current_buffer_offset_);
...
int SeekableBuffer::InternalRead(uint8* data, int size, bool advance_position, int forward_offset)
{
    ...
    while (taken < size) {
        ...
        DataBuffer* buffer = (*current_buffer);
        ...
        if (current_buffer_offset == buffer->data_size()) {
            if (advance_position) {
                ...
                UpdateCurrentTime(current_buffer_ != buffers_.end(), (*current_buffer_), current_buffer_offset);
            }
...
void SeekableBuffer::UpdateCurrentTime(bool isNotEnd, DataBuffer* buffer, int offset)
{
    // Garbage values are unavoidable, so this check will remain.
    if (isNotEnd && (buffer)->timestamp() != base::TimeDelta::FromMicroseconds(kint64min)) {
