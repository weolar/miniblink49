...
void getBrightnessMatrix(float amount, SkScalar* matrix)
{
    // Spec implementation
...
skia::RefPtr<SkImageFilter> createMatrixImageFilter(const SkScalar matrix[20], const skia::RefPtr<SkImageFilter>& input)
{
    skia::RefPtr<SkColorFilter> colorFilter = skia::AdoptRef(SkColorMatrixFilter::Create(matrix));
    return skia::AdoptRef(SkColorFilterImageFilter::Create(colorFilter.get(), input.get()));
}

}  // namespace

skia::RefPtr<SkImageFilter> RasterFilter::buildImageFilter(const mc_blink::WebFilterOperationsImpl* filters, const blink::FloatSize& size)
{
