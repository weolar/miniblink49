...
#ifndef mc_RecordDrawHelp_h
#define mc_RecordDrawHelp_h

#include <wtf/Functional.h>
#include "mc/raster/RasterTaskClient.h"
...
public:
    ...
    void recordDraw(mc_blink::WebLayerImpl* webLayer, OwnPtr<DrawFunction> drawFunction)
    {
        DebugBreak();
        ...
        mc::LayerTreeHost* treeHost = webLayer->layerTreeHost();
        ...
        if (!m_bitmapWrap) {
            m_bitmapWrap = new mc::SkBitmapRefWrap();
            m_bitmapWrap->ref();
        ...
        m_bounds = bounds;
        m_rasterTaskCount++;
        mc::RasterTaskWorkerThreadPool::shared()->postRasterTask(new mc::RasterResouce(m_bitmapWrap->ref(), &m_mutex, picture, layerRect, this));
    }

    void drawToCanvas(mc_blink::WebLayerImpl* webLayer, blink::WebCanvas* canvas, const blink::IntRect& clip)
    {
