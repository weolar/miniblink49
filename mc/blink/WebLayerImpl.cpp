...
static void clearLayerActions(WTF::Vector<mc::LayerChangeAction*>* actions);

#ifndef NDEBUG
DEFINE_DEBUG_ONLY_GLOBAL(WTF::RefCountedLeakCounter, webLayerImplCounter, ("mcWebLayerImpl"));
#endif
...
//     , m_clipParent(nullptr)
    ...
    , m_tileGrid(nullptr)
    , m_drawProperties(new mc::DrawProps())
    , m_isMaskLayer(false)
    ...
    , m_filterOperations(nullptr)
{
    ...
    appendLayerChangeAction(new mc::LayerChangeActionCreate(-1, id(), m_layerType));
...
WebLayerImpl::~WebLayerImpl()
{
    ...
    if (m_layerTreeHost) {
        ...
        m_layerTreeHost->appendLayerChangeAction(new mc::LayerChangeActionDestroy(actionId, id()));
    }
...
mc::LayerTreeHost* WebLayerImpl::layerTreeHost() const
{
...
static void applyLayerActions(mc::LayerTreeHost* host, WTF::Vector<mc::LayerChangeAction*>* actions)
{
    for (size_t i = 0; i < actions->size(); ++i) {
        mc::LayerChangeAction* action = actions->at(i);
...
void WebLayerImpl::setLayerTreeHost(mc::LayerTreeHost* host)
{
    ...
    if (host) {
        ...
        if (m_drawsContent && !m_tileGrid && host->needTileRender())
            m_tileGrid = new mc::TileGrid(this);
...
mc::DrawProps* WebLayerImpl::drawProperties()
{
    return m_drawProperties;
}

void WebLayerImpl::updataDrawToCanvasProperties(mc::DrawProps* prop)
{
...
static void appendChildrenToDirtyLayers(mc::RasterTaskGroup* taskGroup, WebLayerImpl* layer)
{
...
void WebLayerImpl::recordDrawChildren(mc::RasterTaskGroup* taskGroup, int deep)
{
    for (size_t i = 0; i < children().size(); ++i) {
...
void WebLayerImpl::recordDraw(mc::RasterTaskGroup* taskGroup)
{
    if (m_client)
...
void WebLayerImpl::appendLayerChangeAction(mc::LayerChangeAction* action)
{
    if (m_layerTreeHost) {
        ASSERT(-1 == action->actionId());
...
{
    mc::LayerTreeHost* host = layer->layerTreeHost();
...
//     OutputDebugStringW(outString.charactersWithNullTermination().data());
    ...
    appendLayerChangeAction(new mc::LayerChangeActionInsertChild(-1, id(), child->id(), index));
...
void WebLayerImpl::appendLayerActionsToParent()
{
    ...
    WTF::Vector<mc::LayerChangeAction*>* actions = &m_savedActionsWhenHostIsNull;
    for (size_t i = 0; i < actions->size(); ++i) {
        mc::LayerChangeAction* action = actions->at(i);
        m_parent->appendLayerChangeAction(action);
    }
...
void WebLayerImpl::removeFromParent()
{
    if (m_parent) {
        ...
        appendLayerChangeAction(new mc::LayerChangeActionRemoveFromParent(-1, id()));
    }
}
...
{
    if (m_scrollParent == (mc_blink::WebLayerImpl*)parent)
        return;
    m_scrollParent = (mc_blink::WebLayerImpl*)parent;
// 
//     if (m_scrollParent)
...
{
//     if (m_clipParent == (mc_blink::WebLayerImpl*)parent)
//         return;
//     m_clipParent = (mc_blink::WebLayerImpl*)parent;
// 
//     if (m_clipParent)
