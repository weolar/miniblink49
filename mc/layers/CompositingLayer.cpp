...
#include "mc/tiles/CompositingTile.h"
...
#ifndef NDEBUG
DEFINE_DEBUG_ONLY_GLOBAL(WTF::RefCountedLeakCounter, compositingLayerCounter, ("mcCompositingLayer"));
#endif
...
bool CompositingLayer::drawToCanvasChildren(LayerTreeHost* host, SkCanvas* canvas, const SkRect& clip, float parentOpacity, int deep)
{
    ...
    for (size_t i = 0; i < children().size(); ++i) {
        ...
        if (!child->opaque() || !child->masksToBounds() || !child->drawsContent())
            b |= child->drawToCanvasChildren(host, canvas, clip, parentOpacity * opacity(), deep + 1);
