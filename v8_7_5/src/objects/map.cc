...
// }
// 
// BitField<PropertyLocation, KindField::kNext, 1>
...
static constexpr int my_decode(uint32_t value) {
  const uint32_t kMask = ((1u << 1) << 1) - (1u << 1);
  return static_cast<int>((value & kMask) >> 1);
}
