...
static Local<v8::String> CreateV8String(Environment* env, const char* name) {
    return v8::String::NewFromOneByte(env->isolate(), (const uint8_t *)name, v8::String::kNormalString, -1);
}
...
inline url_data HarvestBase(Environment* env, Local<Object> base_obj) {
  ...
  Local<Value> scheme =
      base_obj->Get(env->context(), CreateV8String(env, "scheme")).ToLocalChecked();
  ...
  Local<Value> port =
      base_obj->Get(env->context(), CreateV8String(env, "port")).ToLocalChecked();
  if (port->IsInt32())
...
inline url_data HarvestContext(Environment* env, Local<Object> context_obj) {
  ...
  Local<Value> scheme =
      context_obj->Get(env->context(), CreateV8String(env, "scheme")).ToLocalChecked();
  if (scheme->IsString()) {
    Utf8Value value(env->isolate(), scheme);
    context.scheme.assign(*value, value.length());
  }
  Local<Value> port =
      context_obj->Get(env->context(), CreateV8String(env, "port")).ToLocalChecked();
  if (port->IsInt32())
