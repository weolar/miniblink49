...
#if !defined(WIN32) && !defined(WATT32) && !defined(ANDROID) && !defined(__ANDROID__) && !defined(CARES_USE_LIBRESOLV)
...
#define ARES_CONFIG_CHECK(x) (x->lookups && x->nsort > -1 && x->nservers > -1 && x->ndomains > -1 && x->ndots > -1 && x->timeout > -1 && x->tries > -1)
...
int ares_dup(ares_channel* dest, ares_channel src)
{
    ...
    *dest = NULL; /* in case of failure return NULL explicitly */
    ...
    for (i = 0; i < src->nservers; i++) {
        if ((src->servers[i].addr.family != AF_INET) || (src->servers[i].addr.udp_port != 0) || (src->servers[i].addr.tcp_port != 0)) {
            non_v4_default_port++;
            break;
        }
    }
    ...
    int* optmask)
{
    ...
    (*optmask) = (ARES_OPT_FLAGS | ARES_OPT_TRIES | ARES_OPT_NDOTS | ARES_OPT_UDP_PORT | ARES_OPT_TCP_PORT | ARES_OPT_SOCK_STATE_CB | ARES_OPT_SERVERS | ARES_OPT_DOMAINS | ARES_OPT_LOOKUPS | ARES_OPT_SORTLIST | ARES_OPT_TIMEOUTMS) | (channel->optmask & ARES_OPT_ROTATE);
    ...
    if (channel->nservers) {
        for (i = 0; i < channel->nservers; i++) {
            if ((channel->servers[i].addr.family == AF_INET) && (channel->servers[i].addr.udp_port == 0) && (channel->servers[i].addr.tcp_port == 0))
                ipv4_nservers++;
        }
        if (ipv4_nservers) {
    ...
    int optmask)
{
    ...
    if ((optmask & ARES_OPT_SERVERS) && channel->nservers == -1) {
        ...
        if (options->nservers > 0) {
            channel->servers = ares_malloc(options->nservers * sizeof(struct server_state));
            if (!channel->servers)
                return ARES_ENOMEM;
    ...
    *outptr = NULL;
    ...
    for (ipAddr = &fi->DnsServerList; ipAddr; ipAddr = ipAddr->Next) {
        ...
        if (ares_inet_pton(AF_INET, txtaddr, &namesrvr.addrV4) == 1) {
            if ((namesrvr.addrV4.S_un.S_addr == INADDR_ANY) || (namesrvr.addrV4.S_un.S_addr == INADDR_NONE))
                continue;
        } else if (ares_inet_pton(AF_INET6, txtaddr, &namesrvr.addrV6) == 1) {
            if (memcmp(&namesrvr.addrV6, &ares_in6addr_any,
                    sizeof(namesrvr.addrV6))
                == 0)
                continue;
        } else
            continue;
...
static int init_by_resolv_conf(ares_channel channel)
{
#if !defined(ANDROID) && !defined(__ANDROID__) && !defined(WATT32) && !defined(CARES_USE_LIBRESOLV)
    char* line = NULL;
#endif
    ...
    char* str)
{
   ...
   */
    for (p = str; p;) {
        ...
        newserv = ares_realloc(*servers, (*nservers + 1) * sizeof(struct server_state));
        if (!newserv)
            return ARES_ENOMEM;
    ...
    const char* str)
{
    ...
    while (*str && *str != ';') {
        ...
        } else if (ipbufpfx[0] && (bits = ares_inet_net_pton(AF_INET, ipbufpfx, &pat.addrV4, sizeof(pat.addrV4))) > 0) {
            pat.type = PATTERN_CIDR;
            pat.mask.bits = (unsigned short)bits;
            pat.family = AF_INET;
            if (!sortlist_alloc(sortlist, nsort, &pat)) {
