...
#ifdef TENCENT_CHANGES
      if (GpuBugManager::GetInstance()->use_teximage2d_free_memory() && (target() == GL_TEXTURE_2D)) {
        //TRACE_EVENT0("gpu", "Texture::RemoveTextureRef. glTexImage2D");
        GLenum type;
        GLenum format;
        if (GetLevelType(GL_TEXTURE_2D, 0, &type, &format)) {
          gfx::ScopedTextureBinder texture_binder(GL_TEXTURE_2D, id);
          glTexImage2D(GL_TEXTURE_2D, 0, format, 1, 1, 0, format, type, NULL);

          /*
          GLenum error;
          if ((error = glGetError()) != GL_NO_ERROR) {
            LOG(ERROR) << "Texture::RemoveTextureRef. glGetError:" << error;
          }
          */
        }
      }
