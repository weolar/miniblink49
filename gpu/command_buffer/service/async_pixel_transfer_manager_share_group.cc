...
#ifdef TENCENT_CHANGES
AsyncPixelTransferManagerShareGroup* AsyncPixelTransferManagerShareGroup::Create(gfx::GLContext* context) {
  const int Max_failed_count = 2;
  static int failed_count = 0;
  //如果连续创建context失败，意味这后面成功可能性不大。则直接返回，避免总是线程等待走失败流程。
  //造成这种情况大都是因为gpu不支持sharegroup。
  if (failed_count >= Max_failed_count)
    return NULL;

  g_transfer_thread.Pointer()->InitializeOnMainThread(context);
  if (!g_transfer_thread.Pointer()->IsInitialized()) {
    failed_count++;
    return NULL;
  }

  failed_count = 0;
  return new AsyncPixelTransferManagerShareGroup(context);
}
#endif
