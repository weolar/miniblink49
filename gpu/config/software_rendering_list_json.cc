...
// TODO(Perryuwang): gl_reset_notification_strategy的检查导致大部分机型无法开启WebGL功能，
//   考虑到gl_reset_notification_strategy并不是在所有情况下都会用到，只是在lose context的时候需要。
//   所以先把这个检查忽略掉，测试WebGL的兼容性，后期再看是否有其他办法绕过这个问题。
    {
      "id": 76,
