...
void GpuBugManager::Initialize() {
  ...
  if (!base::android::SysUtils::RunningOnJellyBeanMR1OrHigher() &&
      std::string::npos != gl_vendor.find("ARM")) {
  ...
  if ((!base::android::SysUtils::RunningOnLollipopOrHigher() &&
      std::string::npos != gl_renderer.find("PowerVR SGX 544MP")) ||
      (!base::android::SysUtils::RunningOnJellyBeanOrHigher() &&
      (std::string::npos != gl_renderer.find("PowerVR SGX 540"))) ||
      (std::string::npos != gl_renderer.find("Mali-T624")) ||
      (std::string::npos != gl_renderer.find("Mali-T628")) ||
      (std::string::npos != gl_renderer.find("Vivante GC2000"))) {
  ...
  if (!base::android::SysUtils::RunningOnLollipopOrHigher() &&
      (std::string::npos != gl_renderer.find("Adreno (TM) 330")) &&
      (std::string::npos != gl_version.find("OpenGL ES 3.0"))) {
