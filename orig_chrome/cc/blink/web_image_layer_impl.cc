...
void WebImageLayerImpl::setImage(const SkImage* image)
{
    ...
    static_cast<cc::PictureImageLayer*>(layer_->layer())->SetImage(imageRef /*.Pass()*/);
    static_cast<WebLayerImplFixedBounds*>(layer_.get())->SetFixedBounds(gfx::Size(image->width(), image->height()));
}

void WebImageLayerImpl::setImageBitmap(const SkBitmap& bitmap)
{
    if (0 == bitmap.width() * bitmap.height())
        return;
...
{
    static_cast<cc::PictureImageLayer*>(layer_->layer())->SetNearestNeighbor(nearest_neighbor);
}
