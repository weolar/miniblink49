...
#ifndef NOT_QB_BUILD // huarongxu fro save web page to image
namespace {
base::LazyInstance<base::ThreadLocalPointer<gfx::Size>>
    lazy_tls_size = LAZY_INSTANCE_INITIALIZER;
} // namespace

namespace cc {
gfx::Size* GetThreadLocalSize()
{
    return lazy_tls_size.Pointer()->Get();
}

void SetThreadLocalSize(scoped_ptr<gfx::Size> size)
{
    lazy_tls_size.Pointer()->Set(size.release());
}
...
void Layer::ReplaceChild(Layer* reference, scoped_refptr<Layer> new_layer)
{
    ...
    auto reference_it = std::find_if(children_.begin(), children_.end(),
    ...
    if (layer_tree_host_) {
        if (TransformNode* transform_node = layer_tree_host_->property_trees()->transform_tree.Node(
            ...
            if (transform_node->owner_id == id()) {
                ...
                bool preserves_2d_axis_alignment = Are2dAxisAligned(transform_, transform, &invertible);
...
{
    DCHECK(IsPropertyChangeAllowed());
    if (user_scrollable_horizontal_ == horizontal && user_scrollable_vertical_ == vertical)
        return;
...
{
    if (!layer_tree_host_ || layer_tree_host_->property_trees()->sequence_number != property_tree_sequence_number_) {
        return -1;
    }
...
void Layer::SetNeedsDisplayRect(const gfx::Rect& dirty_rect)
{
    ...
    //   sprintf(outString, "Layer::SetNeedsDisplayRect: id:%d, %d %d\n", id(), dirty_rect.width(), dirty_rect.height());
...
{
    for (size_t i = 0; i < children_.size(); ++i) {
        if (children_[i]->position_constraint_.is_fixed_position() || children_[i]->DescendantIsFixedToContainerLayer())
            return true;
    }
    return false;
}
    ...
    scoped_ptr<CopyOutputResult> result)
{
    ...
    if (GetThreadLocalSize()) {
        scoped_ptr<gfx::Size> size_to_delete(GetThreadLocalSize());
        SetThreadLocalSize(scoped_ptr<gfx::Size>());
    }
...
void Layer::PushPropertiesTo(LayerImpl* layer)
{
    ...
    bool use_paint_properties = paint_properties_.source_frame_number == layer_tree_host_->source_frame_number();
    ...
    if (clip_parent_) {
        clip_parent = layer->layer_tree_impl()->LayerById(clip_parent_->id());
        DCHECK(clip_parent);
    }
    ...
    for (ScopedPtrVector<CopyOutputRequest>::iterator it = copy_requests_.begin();
         ...
         ++it) {
        scoped_refptr<base::SingleThreadTaskRunner> main_thread_task_runner = layer_tree_host()->proxy()->MainThreadTaskRunner();
        ...
        scoped_ptr<CopyOutputRequest> main_thread_request = CopyOutputRequest::CreateRelayRequest(
    ...
    bool parent_affected = layer->parent() && layer->parent()->IsAffectedByPageScale();
...
void Layer::SavePaintProperties()
{
    ...
    paint_properties_.source_frame_number = layer_tree_host_->source_frame_number();
}
...
void Layer::OnTransformAnimated(const gfx::Transform& transform)
{
    ...
    SetNeedsUpdate();
    if (layer_tree_host_) {
        if (TransformNode* node = layer_tree_host_->property_trees()->transform_tree.Node(
                transform_tree_index())) {
            if (node->owner_id == id()) {
...
void Layer::OnTransformIsPotentiallyAnimatingChanged(bool is_animating)
{
    if (!layer_tree_host_)
        return;
    TransformTree& transform_tree = layer_tree_host_->property_trees()->transform_tree;
...
bool Layer::AddAnimation(scoped_ptr<Animation> animation)
{
    ...
    if (animation->target_property() == Animation::SCROLL_OFFSET && !layer_animation_controller_->animation_registrar()->supports_scroll_animations())
        return false;
...
{
    visited_tracker_ = visited ? layer_tree_host()->meta_information_sequence_number() : 0;
}

bool Layer::visited()
{
    return visited_tracker_ == layer_tree_host()->meta_information_sequence_number();
}

void Layer::set_layer_or_descendant_is_drawn(
    bool layer_or_descendant_is_drawn)
{
    layer_or_descendant_is_drawn_tracker_ = layer_or_descendant_is_drawn
...
{
    return layer_or_descendant_is_drawn_tracker_ == layer_tree_host()->meta_information_sequence_number();
}

void Layer::set_sorted_for_recursion(bool sorted_for_recursion)
{
    sorted_for_recursion_tracker_ = sorted_for_recursion
        ? layer_tree_host()->meta_information_sequence_number()
        : 0;
}
...
{
    return sorted_for_recursion_tracker_ == layer_tree_host()->meta_information_sequence_number();
}
