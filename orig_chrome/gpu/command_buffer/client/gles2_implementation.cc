...
namespace gles2 {
        ...
        , weak_ptr_factory_(this)
    {
        ...
        GPU_CLIENT_LOG_CODE_BLOCK({
            debug_ = false; // base::CommandLine::ForCurrentProcess()->HasSwitch(switches::kEnableGPUClientLogging);
        });

        share_group_ = (share_group ? share_group
    ...
    void GLES2Implementation::PixelStorei(GLenum pname, GLint param)
    {
        ...
        case GL_PACK_REVERSE_ROW_ORDER_ANGLE:
            pack_reverse_row_order_ = IsAnglePackReverseRowOrderAvailable() ? (param != 0) : false;
            break;
        default:
            break;
        }
        ...
        const void* pixels)
    {
        ...
        pixels = reinterpret_cast<const int8*>(pixels) + unpack_skip_rows_ * src_padded_row_size;
        if (unpack_skip_pixels_) {
            uint32 group_size = GLES2Util::ComputeImageGroupSize(format, type);
            pixels = reinterpret_cast<const int8*>(pixels) + unpack_skip_pixels_ * group_size;
        }
        ...
        const void* pixels)
    {
        ...
        pixels = reinterpret_cast<const int8*>(pixels) + unpack_skip_images_ * src_padded_row_size * src_height + unpack_skip_rows_ * src_padded_row_size;
        if (unpack_skip_pixels_) {
            uint32 group_size = GLES2Util::ComputeImageGroupSize(format, type);
            pixels = reinterpret_cast<const int8*>(pixels) + unpack_skip_pixels_ * group_size;
        }
        ...
        if (buffer_pointer) {
            for (GLsizei z = 0; z < depth; ++z) {
                ...
                uint32 src_unpadded_row_size = (z == depth - 1) ? unpadded_row_size : src_padded_row_size;
                CopyRectToBuffer(
                    pixels, height, src_unpadded_row_size, src_padded_row_size,
                    buffer_pointer, padded_row_size);
                pixels = reinterpret_cast<const int8*>(pixels) + src_padded_row_size * src_height;
                buffer_pointer = reinterpret_cast<int8*>(buffer_pointer) + padded_row_size * height;
            }
            helper_->TexImage3D(
                target, level, internalformat, width, height, depth, format, type,
                shm_id, shm_offset);
    ...
    const GLubyte* GLES2Implementation::GetStringHelper(GLenum name)
    {
        ...
        if (GetBucketAsString(kResultBucketId, &str)) {
            ...
            if (it == gl_strings_.end()) {
                ...
                std::pair<GLStringMap::iterator, bool> insert_result = gl_strings_.insert(std::make_pair(name, strings));
            ...
            if (sit != string_set.end()) {
                ...
                std::pair<std::set<std::string>::const_iterator, bool> insert_result = string_set.insert(str);
                DCHECK(insert_result.second);
        ...
        GLuint valuebuffer)
    {
        ...
        // TODO(gman): See note #2 above.
        if (changed) {
            GetIdHandler(id_namespaces::kValuebuffers)->MarkAsUsedForBind(this, target, valuebuffer, &GLES2Implementation::BindValuebufferCHROMIUMStub);
        }
    }
    ...
    const GLchar* GLES2Implementation::GetRequestableExtensionsCHROMIUM()
    {
        ...
        if (GetBucketAsString(kResultBucketId, &str)) {
            ...
            // the unique results.
            std::set<std::string>::const_iterator sit = requestable_extensions_set_.find(str);
            if (sit != requestable_extensions_set_.end()) {
                result = sit->c_str();
            } else {
                std::pair<std::set<std::string>::const_iterator, bool> insert_result = requestable_extensions_set_.insert(str);
                DCHECK(insert_result.second);
                result = insert_result.first->c_str();
            }
        }
        ...
        if (sync_token) {
            ...
            if (sync_token_data.HasData()) {
                ...
                if (namespace_id == gpu::CommandBufferNamespace::OLD_SYNC_POINTS) {
                    const uint32_t sync_point = static_cast<uint32_t>(sync_token_data.release_count());
        ...
        size_t* out_transforms_offset)
    {
        ...
        uint32 path_name_size = GLES2Util::GetGLTypeSizeForGLPathNameType(path_name_type);
