...
void CommandBufferHelper::CalcImmediateEntries(int waiting_count)
{
    ...
    if (flush_automatically_) {
        int32 limit = total_entry_count_ / ((curr_get == last_put_sent_) ? kAutoFlushSmall : kAutoFlushBig);

        int32 pending = (put_ + total_entry_count_ - last_put_sent_) % total_entry_count_;
