...
#pragma pack(pop)
    ...
    protected:
        ...
        static const int32 kCommandBufferSizeBytes = kNumCommandEntries * sizeof(CommandBufferEntry);
...
// GCC requires these declarations, but MSVC requires they not be present
        ...
        for (int sub = 0; sub < 2; ++sub) {
            for (size_t a = 0; a < arraysize(unpack_alignments); ++a) {
                ...
                for (int y = 0; y < kSrcSubImageHeight; ++y) {
                    const uint8* src_row = src_pixels.get() + (kSrcSubImageY0 + y) * src_padded_row_size + bytes_per_group * kSrcSubImageX0;
    ...
    TEST_F(GLES2ImplementationTest, TexImage3DViaTexSubImage3D)
    {
        ...
        uint32 second_size = padded_row_size * 3 - (padded_row_size - unpadded_row_size);
