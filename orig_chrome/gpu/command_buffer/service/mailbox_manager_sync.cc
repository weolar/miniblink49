...
namespace gles2 {
    ...
    namespace {
        ...
        base::LazyInstance<SyncTokenToFenceMap> g_sync_point_to_fence = LAZY_INSTANCE_INITIALIZER;
        base::LazyInstance<std::queue<SyncTokenToFenceMap::iterator>> g_sync_points = LAZY_INSTANCE_INITIALIZER;
#endif
        ...
        void CreateFenceLocked(const SyncToken& sync_token)
        {
            ...
            if (sync_token.release_count()) {
                ...
                if (fence.get()) {
                    std::pair<SyncTokenToFenceMap::iterator, bool> result = sync_point_to_fence.insert(std::make_pair(sync_token, fence));
...
#if !defined(OS_MACOSX)
            g_lock.Get().AssertAcquired();
            SyncTokenToFenceMap::iterator fence_it = g_sync_point_to_fence.Get().find(sync_token);
    ...
    {
        g_lock.Get().AssertAcquired();
        //DCHECK(std::find(textures_.begin(), textures_.end(), std::make_pair(manager, texture)) == textures_.end());
        for (TextureList::iterator it = textures_.begin(); it != textures_.end(); ++it) {
            if (it->first == manager && it->second == texture)
                DebugBreak();
        }
    ...
    {
        g_lock.Get().AssertAcquired();
        //TextureGroup::TextureList::iterator tex_list_it = std::find(textures_.begin(), textures_.end(), std::make_pair(manager, texture));
        TextureGroup::TextureList::iterator tex_list_it = textures_.begin();
        for (; tex_list_it != textures_.end(); ++tex_list_it) {
            if (tex_list_it->first == manager && tex_list_it->second == texture)
                break;
        }
