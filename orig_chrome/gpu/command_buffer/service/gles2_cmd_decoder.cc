...
#undef GLES2_CMD_OP
    ...
    GLES2DecoderImpl::GLES2DecoderImpl(ContextGroup* group)
        ...
        , gpu_decoder_category_((const unsigned char*)"gpu_decoder"
        ...
        GLenum target, const char* func_name)
    {
        ...
        if (completeness != GL_FRAMEBUFFER_COMPLETE) {
            LOCAL_SET_GL_ERROR(GL_INVALID_FRAMEBUFFER_OPERATION, func_name, "framebuffer incomplete");
            return false;
        }
        ...
        if (renderbuffer_manager()->HaveUnclearedRenderbuffers() || texture_manager()->HaveUnclearedMips()) {
            if (!framebuffer->IsCleared()) {
                // Can we clear them?
                GLenum status = framebuffer->GetStatus(texture_manager(), target);
                if (status != GL_FRAMEBUFFER_COMPLETE) {
                    //LOCAL_SET_GL_ERROR(GL_INVALID_FRAMEBUFFER_OPERATION, func_name, "framebuffer incomplete (clear)");
                    return false;
                }
        ...
        if (!framebuffer_manager()->IsComplete(framebuffer)) {
            if (framebuffer->GetStatus(texture_manager(), target) != GL_FRAMEBUFFER_COMPLETE) {
                LOCAL_SET_GL_ERROR(GL_INVALID_FRAMEBUFFER_OPERATION, func_name, "framebuffer incomplete (check)");
                return false;
            }
    ...
    typedef void(__stdcall* FN_glVertexAttribDivisor)(GLuint index, GLuint divisor);
    ...
    void GLES2DecoderImpl::ClearAllAttributes() const
    {
        ...
        FN_glVertexAttribDivisor ptr = (FN_glVertexAttribDivisor)GetProcAddress(::GetModuleHandle(L"libGLESv2.dll"), "glVertexAttribDivisor");
    ...
    void GLES2DecoderImpl::RenderWarning(const char* filename, int line, const std::string& msg)
    {
        ...
        std::string msg_str = "GLES2DecoderImpl::RenderWarning:";
        msg_str += filename;
        msg_str += ", ";
        msg_str += msg;
        msg_str += "\n";
    ...
    void GLES2DecoderImpl::PerformanceWarning(const char* filename, int line, const std::string& msg)
    {
        ...
        std::string msg_str = "GLES2DecoderImpl::PerformanceWarning:";
        msg_str += filename;
        msg_str += ", ";
        msg_str += msg;
        msg_str += "\n";
            ...
            for (size_t jj = 0; jj < uniform_info->texture_units.size(); ++jj) {
                ...
                if (texture_unit_index < state_.texture_units.size()) {
                    ...
                    if (textarget != GL_TEXTURE_CUBE_MAP) {
                        ...
                        Texture::ImageState image_state;
                        gl::GLImage* image = texture->GetLevelImage(textarget, 0, &image_state);
                        if (image && image_state == Texture::UNBOUND && !texture->IsAttachedToFramebuffer()) {
                            ScopedGLErrorSuppressor suppressor(
        ...
        const void* cmd_data)
    {
        ...
        Texture::ImageState image_state;
        gl::GLImage* image = ref->texture()->GetLevelImage(ref->texture()->target(), 0, &image_state);
    ...
    error::Error GLES2DecoderImpl::HandleScheduleCALayerCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::ScheduleCALayerCHROMIUM& c = *static_cast<const gles2::cmds::ScheduleCALayerCHROMIUM*>(cmd_data);
        ...
        GLint border)
    {
        ...
        if (feature_info_->IsES3Enabled()) {
            ...
            if (color_encoding != GetColorEncodingFromInternalFormat(internal_format) || GLES2Util::IsFloatFormat(internal_format) || (GLES2Util::IsSignedIntegerFormat(internal_format) != GLES2Util::IsSignedIntegerFormat(read_format)) || (GLES2Util::IsUnsignedIntegerFormat(internal_format) != GLES2Util::IsUnsignedIntegerFormat(read_format))) {
                LOCAL_SET_GL_ERROR(
                    GL_INVALID_OPERATION, "glCopyTexImage2D", "incompatible format");
                return;
            }
    ...
    error::Error GLES2DecoderImpl::HandleInsertFenceSyncCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::InsertFenceSyncCHROMIUM& c = *static_cast<const gles2::cmds::InsertFenceSyncCHROMIUM*>(cmd_data);
    ...
    error::Error GLES2DecoderImpl::HandleWaitSyncTokenCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::WaitSyncTokenCHROMIUM& c = *static_cast<const gles2::cmds::WaitSyncTokenCHROMIUM*>(cmd_data);

        const gpu::CommandBufferNamespace namespace_id = static_cast<gpu::CommandBufferNamespace>(c.namespace_id);
        ...
        GLsizei height)
    {
        ...
        // Try using GLImage::CopyTexSubImage when possible.
        if (image && image->CopyTexSubImage(GL_TEXTURE_2D, gfx::Point(xoffset, yoffset), gfx::Rect(x, y, width, height))) {
            return;
        }
    ...
    public:
            ...
            const GLfloat** out_transforms)
        {
            ...
            uint32 transforms_shm_offset = static_cast<uint32>(cmd.transformValues_shm_offset);
            uint32 transforms_component_count = GLES2Util::GetComponentCountForGLTransformType(transform_type);
        ...
        const void* cmd_data)
    {
        ...
        GLuint mask = 0;
        GLenum cover_mode = GL_BOUNDING_BOX_CHROMIUM;
        if (!v.GetFillModeAndMask(c, &fill_mode, &mask) || !v.GetCoverMode(c, &cover_mode))
            return v.error();
    ...
    error::Error GLES2DecoderImpl::HandleStencilFillPathInstancedCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::StencilFillPathInstancedCHROMIUM& c = *static_cast<const gles2::cmds::StencilFillPathInstancedCHROMIUM*>(
            cmd_data);
        if (!features().chromium_path_rendering)
            return error::kUnknownCommand;
        ...
        if (!v.GetPathCountAndType(c, &num_paths, &path_name_type) || !v.GetFillModeAndMask(c, &fill_mode, &mask) || !v.GetTransformType(c, &transform_type))
            return v.error();
    ...
    error::Error GLES2DecoderImpl::HandleStencilStrokePathInstancedCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::StencilStrokePathInstancedCHROMIUM& c = *static_cast<const gles2::cmds::StencilStrokePathInstancedCHROMIUM*>(
            cmd_data);
        if (!features().chromium_path_rendering)
            return error::kUnknownCommand;
        ...
        GLenum path_name_type = GL_NONE;
        GLenum transform_type = GL_NONE;
        if (!v.GetPathCountAndType(c, &num_paths, &path_name_type) || !v.GetTransformType(c, &transform_type))
            return v.error();
    ...
    error::Error GLES2DecoderImpl::HandleCoverFillPathInstancedCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::CoverFillPathInstancedCHROMIUM& c = *static_cast<const gles2::cmds::CoverFillPathInstancedCHROMIUM*>(
            cmd_data);
        if (!features().chromium_path_rendering)
            return error::kUnknownCommand;
        ...
        GLenum transform_type = GL_NONE;
        if (!v.GetPathCountAndType(c, &num_paths, &path_name_type) || !v.GetCoverMode(c, &cover_mode) || !v.GetTransformType(c, &transform_type))
            return v.error();
    ...
    error::Error GLES2DecoderImpl::HandleCoverStrokePathInstancedCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::CoverStrokePathInstancedCHROMIUM& c = *static_cast<const gles2::cmds::CoverStrokePathInstancedCHROMIUM*>(
            cmd_data);
        if (!features().chromium_path_rendering)
            return error::kUnknownCommand;
    ...
    error::Error GLES2DecoderImpl::HandleStencilThenCoverFillPathInstancedCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::StencilThenCoverFillPathInstancedCHROMIUM& c = *static_cast<
        ...
        GLenum cover_mode = GL_BOUNDING_BOX_OF_BOUNDING_BOXES_CHROMIUM;
        GLenum transform_type = GL_NONE;
        if (!v.GetPathCountAndType(c, &num_paths, &path_name_type) || !v.GetFillModeAndMask(c, &fill_mode, &mask) || !v.GetCoverMode(c, &cover_mode) || !v.GetTransformType(c, &transform_type))
            return v.error();
    ...
    GLES2DecoderImpl::HandleStencilThenCoverStrokePathInstancedCHROMIUM(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::StencilThenCoverStrokePathInstancedCHROMIUM& c = *static_cast<
        ...
        const char* name)
    {
        ...
        if (location < 0 || static_cast<uint32>(location) >= group_->max_varying_vectors() * 4) {
    ...
    error::Error GLES2DecoderImpl::HandleBindFragmentInputLocationCHROMIUMBucket(
        ...
        const void* cmd_data)
    {
        const gles2::cmds::BindFragmentInputLocationCHROMIUMBucket& c = *static_cast<const gles2::cmds::BindFragmentInputLocationCHROMIUMBucket*>(
            cmd_data);
        if (!features().chromium_path_rendering) {
            return error::kUnknownCommand;
        }
        ...
        const void* cmd_data)
    {
        ...
        const gles2::cmds::ProgramPathFragmentInputGenCHROMIUM& c = *static_cast<const gles2::cmds::ProgramPathFragmentInputGenCHROMIUM*>(
            cmd_data);
        if (!features().chromium_path_rendering) {
            return error::kUnknownCommand;
        }
        ...
        if ((components != 0 && gen_mode == GL_NONE) || (components == 0 && gen_mode != GL_NONE)) {
            LOCAL_SET_GL_ERROR(GL_INVALID_VALUE, kFunctionName,
        ...
        const Program::FragmentInputInfo* fragment_input_info = program->GetFragmentInputInfoByFakeLocation(location);
        ...
        if (components > 0) {
            ...
            uint32 coeffs_per_component = GLES2Util::GetCoefficientCountForGLPathFragmentInputGenMode(gen_mode);
