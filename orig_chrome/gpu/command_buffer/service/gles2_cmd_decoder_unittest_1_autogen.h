...
{
    EXPECT_CALL(*gl_, BindBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, 2, kServiceBufferId, 4, 4));
...
{
    EXPECT_CALL(*gl_, BindBufferRange(GL_TRANSFORM_FEEDBACK_BUFFER, 2, kNewServiceId, 4, 4));
    EXPECT_CALL(*gl_, GenBuffersARB(1, _))
        .WillOnce(SetArgumentPointee<1>(kNewServiceId));
    SpecializedSetup<cmds::BindBufferRange, 0>(true);
    cmds::BindBufferRange cmd;
...
TEST_P(GLES2DecoderTest1, GetFramebufferAttachmentParameterivInvalidArgs0_0)
{
    ...
    cmds::GetFramebufferAttachmentParameteriv::Result* result = static_cast<cmds::GetFramebufferAttachmentParameteriv::Result*>(
...
TEST_P(GLES2DecoderTest1, GetIntegeri_vValidArgs)
{
    ...
    Result* result = static_cast<Result*>(shared_memory_address_);
    EXPECT_CALL(*gl_, GetIntegeri_v(GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, 2, result->GetData()));
    result->size = 0;
...
TEST_P(GLES2DecoderTest1, GetInteger64i_vValidArgs)
{
    ...
    Result* result = static_cast<Result*>(shared_memory_address_);
    EXPECT_CALL(*gl_, GetInteger64i_v(GL_TRANSFORM_FEEDBACK_BUFFER_BINDING, 2, result->GetData()));
    result->size = 0;
