...
//   }
    ...
    void Program::UpdateFragmentInputs()
    {
        ...
        for (GLint ii = 0; ii < num_fragment_inputs; ++ii) {
            ...
            if (varying && varying->findInfoByMappedName(service_name, &info, &client_name)) {
                type = info->type;
            ...
            if (it != bind_fragment_input_location_map_.end() && it->second >= 0 && query_results[0] >= 0) {
                ...
                fragment_input_infos_[client_location] = FragmentInputInfo(type, service_location);
            }
            ...
            if (!parsed_client_name.IsArrayName() || parsed_client_name.element_index() != 0 || !parsed_service_name.IsArrayName() || parsed_service_name.element_index() != 0) {
            ...
            for (GLsizei jj = 1; jj < size; ++jj) {
                ...
                std::string client_element_name = parsed_client_name.base_name() + array_spec;
                ...
                if (it != bind_fragment_input_location_map_.end() && it->second >= 0) {
                    size_t client_location = static_cast<size_t>(it->second);
                    std::string service_element_name = parsed_service_name.base_name() + array_spec;
                    ...
                    if (service_location >= 0) {
                        if (fragment_input_infos_.size() <= client_location)
                            fragment_input_infos_.resize(client_location + 1);
                        DCHECK(!fragment_input_infos_[client_location].IsValid());
                        fragment_input_infos_[client_location] = FragmentInputInfo(type, static_cast<GLuint>(service_location));
                    }
                }
            }
        }
    ...
    {
        if (fake_location < 0 || static_cast<size_t>(fake_location) >= fragment_input_infos_.size())
            return nullptr;
        ...
        for (const VarT& field : fields) {
            ...
            if (field.isStruct()) {
                for (unsigned int array_element = 0; array_element < field.elementCount();
                    ...
                    const std::string uniform_element_name = full_name + (field.isArray() ? array_string : "");
