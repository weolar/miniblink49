...
namespace gles2 {
    ...
    namespace {
        ...
        public:
            ...
            {
                std::vector<std::string> tokens = base::SplitString(str, " ", base::KEEP_WHITESPACE, base::SPLIT_WANT_NONEMPTY);
                //string_set_.insert(tokens.begin(), tokens.end());
                for (size_t i = 0; i < tokens.size(); ++i) {
                    string_set_.insert(tokens[i]);
                }
            }
...
#undef GPU_OP
    ...
    void FeatureInfo::InitializeBasicState(const base::CommandLine* command_line)
    {
        ...
        //   }
        feature_flags_.enable_shader_name_hashing = true; // !command_line->HasSwitch(switches::kDisableShaderNameHashing);

        feature_flags_.is_swiftshader = false; // (command_line->GetSwitchValueASCII(switches::kUseGL) == "swiftshader");

        feature_flags_.enable_subscribe_uniform = false; // command_line->HasSwitch(switches::kEnableSubscribeUniformExtension);

        enable_unsafe_es3_apis_switch_ = false; // command_line->HasSwitch(switches::kEnableUnsafeES3APIs);

        enable_gl_path_rendering_switch_ = false; // command_line->HasSwitch(switches::kEnableGLPathRendering);

        // The shader translator is needed to translate from WebGL-conformant GLES SL
        // to normal GLES SL, enforce WebGL conformance, translate from GLES SL 1.0 to
        // target context GLSL, etc.
        // The flag here is for testing only.
        disable_shader_translator_ = false; // command_line->HasSwitch(switches::kDisableGLSLTranslator);
    ...
    void FeatureInfo::InitializeFeatures()
    {
        ...
        // GL_EXT_sRGB.
        if (!workarounds_.disable_ext_srgb && (((gl_version_info_->is_es3 || extensions.Contains("GL_OES_rgb8_rgba8")) && extensions.Contains("GL_EXT_sRGB")) || gfx::HasDesktopGLFeatures())) {
        ...
        if (!workarounds_.disable_angle_instanced_arrays && (extensions.Contains("GL_ANGLE_instanced_arrays") || (extensions.Contains("GL_ARB_instanced_arrays") && extensions.Contains("GL_ARB_draw_instanced")) || gl_version_info_->is_es3 || gl_version_info_->is_desktop_core_profile)) {
        ...
        if (enable_gl_path_rendering_switch_ && !workarounds_.disable_gl_path_rendering && extensions.Contains("GL_NV_path_rendering")) {
            bool has_dsa = gl_version_info_->IsAtLeastGL(4, 5) || extensions.Contains("GL_EXT_direct_state_access");
            bool has_piq = gl_version_info_->IsAtLeastGL(4, 3) || extensions.Contains("GL_ARB_program_interface_query");
            if (gl_version_info_->IsAtLeastGLES(3, 1) || (gl_version_info_->IsAtLeastGL(3, 2) && has_dsa && has_piq)) {
