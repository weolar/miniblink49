    ...
    const MemoryMappedFile::Region& region)
{
    ...
    if (region == MemoryMappedFile::Region::kWholeFile) {
        ...
        if (aligned_start < 0 || aligned_size < 0 || aligned_start > std::numeric_limits<off_t>::max() || static_cast<uint64>(aligned_size) > std::numeric_limits<size_t>::max() || static_cast<uint64>(region.size) > std::numeric_limits<size_t>::max()) {
