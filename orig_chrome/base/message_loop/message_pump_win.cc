    ...
    HWND hwnd, UINT message, WPARAM wparam, LPARAM lparam)
{
    switch (message) {
    case kMsgHaveWork:
        if (s_disableDispatchMessageReentry == ::GetCurrentThreadId())
            break;
...
void MessagePumpForUI::RunUntilIdleWithoutMsgPeek(Delegate* delegate)
{
    RunState s;
    s.delegate = delegate;
...
void MessagePumpForUI::WaitForWork()
{
    ...
    if (WAIT_OBJECT_0 == result) {
        ...
        if (HIWORD(queue_status) & QS_MOUSE && !PeekMessage(&msg, NULL, WM_MOUSEFIRST, WM_MOUSELAST, PM_NOREMOVE)) {
            WaitMessage();
        }
        return;
    }
...
{
    TRACE_EVENT1("base", "MessagePumpForUI::ProcessMessageHelper", "message", msg.message);
...
bool MessagePumpForUI::ProcessPumpReplacementMessage()
{
    ...
    if (MessageLoop::current()->os_modal_loop()) {
        ...
        have_message = PeekMessage(&msg, NULL, WM_PAINT, WM_PAINT, PM_REMOVE) || PeekMessage(&msg, NULL, WM_TIMER, WM_TIMER, PM_REMOVE);
    } else {
...
bool MessagePumpForIO::WaitForIOCompletion(DWORD timeout, IOHandler* filter)
{
    ...
    if (!item.has_valid_io_context || item.context->handler) {
        if (filter && item.handler != filter) {
            ...
            DCHECK(!item.has_valid_io_context || (item.context->handler == item.handler));
            WillProcessIOEvent();
...
// static
ULONG_PTR MessagePumpForIO::HandlerToKey(IOHandler* handler, bool has_valid_io_context)
{
...
// static
MessagePumpForIO::IOHandler* MessagePumpForIO::KeyToHandler(ULONG_PTR key, bool* has_valid_io_context)
{
