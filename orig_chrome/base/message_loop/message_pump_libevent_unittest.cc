...
#define MAYBE_TestWatchingFromBadThread TestWatchingFromBadThread
    ...
    public:
        ...
        {
            RunLoop runloop;
            MessageLoop::current()->PostTask(FROM_HERE, Bind(&QuitMessageLoopAndStart, runloop.QuitClosure()));
            runloop.Run();
        }
    ...
    TEST_F(MessagePumpLibeventTest, QuitWatcher)
    {
        ...
        io_loop()->PostTask(FROM_HERE, Bind(&WaitableEventWatcher::StopWatching, Owned(watcher.release())));
    }
