...
inline bool operator<(const Location& _X, const Location& _Y)
{
    size_t v1 = base::HashPair(reinterpret_cast<uint64_t>(_X.file_name()), _X.line_number());
    size_t v2 = base::HashPair(reinterpret_cast<uint64_t>(_Y.file_name()), _Y.line_number());
    return v1 < v2;
}
...
struct hash<tracked_objects::Location> {
    size_t operator()(tracked_objects::Location key) const
    {
        return tracked_objects::Location::Hash()(key);
    }
};
