...
namespace {
    ...
    TEST(JSONValueDeserializerTest, ReadProperJSONFromString)
    {
        ...
        scoped_ptr<Value> value = str_deserializer.Deserialize(&error_code, &error_message);
    ...
    TEST(JSONValueDeserializerTest, ReadProperJSONFromFile)
    {
        ...
        int error_code = 0;
        std::string error_message;
        scoped_ptr<Value> value = file_deserializer.Deserialize(&error_code, &error_message);
        ASSERT_TRUE(value.get());
        ASSERT_EQ(0, error_code);
        ASSERT_TRUE(error_message.empty());
        // Verify if the same JSON is still there.
...
#undef JSON_NEWLINE
    ...
    TEST_F(JSONFileValueSerializerTest, Roundtrip)
    {
        ...
        original_file_path = original_file_path.Append(FILE_PATH_LITERAL("serializer_test.json"));
        ...
        // Now try writing.
        const base::FilePath written_file_path = temp_dir_.path().Append(FILE_PATH_LITERAL("test_output.js"));
