...
namespace internal {
    ...
    namespace {
        ...
        public:
            ...
            bool Equals(const Value* other) const override
            {
                ...
                return other->IsType(TYPE_STRING) && other->GetAsString(&other_string) && StringPiece(other_string) == string_piece_;
            }
    ...
    Value* JSONParser::Parse(const StringPiece& input)
    {
        ...
        if (CanConsume(3) && static_cast<uint8>(*pos_) == 0xEF && static_cast<uint8>(*(pos_ + 1)) == 0xBB && static_cast<uint8>(*(pos_ + 2)) == 0xBF) {
            NextNChars(3);
        }
