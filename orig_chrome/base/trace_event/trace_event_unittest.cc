...
#define EXPECT_SUB_FIND_(string) \
    ...
    TEST_F(TraceEventTestFixture, AddMetadataEvent)
    {
        ...
        scoped_refptr<ConvertableToTraceFormat> convertable = new Convertable(&num_calls);
    ...
    TEST_F(TraceEventTestFixture, Categories)
    {
        ...
        EXPECT_TRUE(std::find(cat_groups.begin(),
            ...
            != cat_groups.end());
        EXPECT_TRUE(std::find(cat_groups.begin(),
                        cat_groups.end(), "c5,c6")
            != cat_groups.end());
        EXPECT_TRUE(std::find(cat_groups.begin(),
                        cat_groups.end(), "c7,c8")
            != cat_groups.end());
        EXPECT_TRUE(std::find(cat_groups.begin(),
                        cat_groups.end(),
                        "disabled-by-default-c9")
            != cat_groups.end());
        // Make sure metadata isn't returned.
        EXPECT_TRUE(std::find(cat_groups.begin(),
                        cat_groups.end(), "__metadata")
            == cat_groups.end());
