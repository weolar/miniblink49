...
namespace {
    ...
    uint64 GetTaskTraceID(const SequencedTask& task,
        void* pool)
    {
        return (static_cast<uint64>(task.trace_id) << 32) | static_cast<uint64>(reinterpret_cast<intptr_t>(pool));
    }
...
bool SequencedWorkerPool::PostDelayedWorkerTask(
    ...
    TimeDelta delay)
{
    WorkerShutdown shutdown_behavior = delay == TimeDelta() ? BLOCK_SHUTDOWN : SKIP_ON_SHUTDOWN;
