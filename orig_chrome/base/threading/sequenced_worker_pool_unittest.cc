...
namespace {
    ...
    TEST_F(SequencedWorkerPoolTest, DISABLED_IgnoresAfterShutdown)
    {
        ...
        for (size_t i = 0; i < kNumWorkerThreads; i++) {
            EXPECT_TRUE(std::find(result.begin(), result.end(), static_cast<int>(i)) != result.end());
        }
    ...
    TEST_F(SequencedWorkerPoolTest, IsRunningOnCurrentThread)
    {
        ...
        scoped_refptr<SequencedWorkerPool> unused_pool = new SequencedWorkerPool(2, "unused_pool");
    ...
    namespace {
        ...
        {
            SequencedWorkerPool::SequenceToken token = SequencedWorkerPool::GetSequenceTokenForCurrentThread();
            EXPECT_EQ(expected_token.ToString(), token.ToString());

            scoped_refptr<SequencedWorkerPool> pool = SequencedWorkerPool::GetWorkerPoolForCurrentThread();
    ...
    TEST_F(SequencedWorkerPoolTest, GetWorkerPoolAndSequenceTokenForCurrentThread)
    {
        ...
        SequencedWorkerPool::SequenceToken local_token = SequencedWorkerPool::GetSequenceTokenForCurrentThread();
        scoped_refptr<SequencedWorkerPool> local_pool = SequencedWorkerPool::GetWorkerPoolForCurrentThread();
