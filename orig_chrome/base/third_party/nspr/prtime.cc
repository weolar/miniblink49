...
PR_ImplodeTime(const PRExplodedTime* exploded)
{
    ...
    PRTime result = static_cast<PRTime>((uli.QuadPart / 10) - 11644473600000000i64);
    // Adjust for time zone and dst.  Convert from seconds to microseconds.
    result -= (exploded->tm_params.tp_gmt_offset + exploded->tm_params.tp_dst_offset) * kSecondsToMicroseconds;
    ...
    CFAbsoluteTime absolute_time = CFGregorianDateGetAbsoluteTime(gregorian_date, NULL);
    PRTime result = static_cast<PRTime>(absolute_time);
    result -= exploded->tm_params.tp_gmt_offset + exploded->tm_params.tp_dst_offset;
    ...
    if (absolute_time == -1 && !(exploded->tm_year == 1969 && exploded->tm_month == 11 && exploded->tm_mday == 31 && exploded->tm_hour == 23 && exploded->tm_min == 59 && exploded->tm_sec == 59)) {
...
void PR_NormalizeTime(PRExplodedTime* time, PRTimeParamFn params)
{
    ...
    time->tm_yday = time->tm_mday + lastDayOfMonth[IsLeapYear(time->tm_year)][time->tm_month];
