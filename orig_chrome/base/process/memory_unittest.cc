...
TEST(ProcessMemoryTest, GetModuleFromAddress)
{
    ...
    HMODULE kernel32_from_address = base::GetModuleFromAddress(reinterpret_cast<DWORD*>(kernel32) + 1);
...
TEST(ProcessMemoryTest, EnableLFH)
{
    ...
    for (unsigned i = 0; i < number_heaps; ++i) {
        ULONG flag = 0;
        SIZE_T length;
        ASSERT_NE(0, heap_query(heaps[i], HeapCompatibilityInformation, &flag, sizeof(flag), &length));
