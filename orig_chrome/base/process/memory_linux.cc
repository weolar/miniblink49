...
__attribute__((weak, visibility("default"))) void* tc_malloc_skip_new_handler_weak(size_t size);
...
#if !defined(ADDRESS_SANITIZER) && !defined(MEMORY_SANITIZER) && !defined(THREAD_SANITIZER) && !defined(LEAK_SANITIZER)
...
{
#if defined(MEMORY_TOOL_REPLACES_ALLOCATOR) || (!defined(LIBC_GLIBC) && !defined(USE_TCMALLOC))
    *result = malloc(size);
