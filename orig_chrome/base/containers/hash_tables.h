...
#define DEFINE_TRIVIAL_HASH(integral_type)           \
    ...
    struct hash<integral_type> {                     \
        size_t operator()(integral_type value) const \
        {                                            \
...
#if USING_VC6RT != 1
    ,
    class Hash = std::hash<Key>, class Pred = std::equal_to<Key>, class Alloc = std::allocator<std::pair<const Key, T>>
#endif
    >
...
#if USING_VC6RT != 1
    ,
    class Hash = std::hash<Key>, class Pred = std::equal_to<Key>, class Alloc = std::allocator<Key>
#endif
    >
...
template <class Key, class T,
    ...
    class Pred = std::equal_to<Key>,
    class Alloc = std::allocator<std::pair<const Key, T>>>
using hash_multimap = BASE_HASH_IMPL_NAMESPACE::hash_multimap<Key, T, Hash, Pred, Alloc>;

template <class Key,
    class Hash = BASE_HASH_NAMESPACE::hash<Key>,
    class Pred = std::equal_to<Key>,
    class Alloc = std::allocator<Key>>
using hash_multiset = BASE_HASH_IMPL_NAMESPACE::hash_multiset<Key, Hash, Pred, Alloc>;
...
inline size_t HashInts32(uint32 value1, uint32 value2)
{
    ...
    size_t high_bits = static_cast<size_t>(
        hash64 >> (8 * (sizeof(uint64) - sizeof(size_t))));
    return high_bits;
}
...
inline size_t HashInts64(uint64 value1, uint64 value2)
{
...
#define DEFINE_32BIT_PAIR_HASH(Type1, Type2)           \
    inline size_t HashPair(Type1 value1, Type2 value2) \
    {                                                  \
