...
TEST(TextEliderTest, MAYBE_ElideEmailMoreSpace)
{
    ...
    for (size_t i = 0; i < arraysize(test_width_factors); ++i) {
        const int test_width = font_list.GetExpectedTextWidth(test_width_factors[i]);
...
TEST(TextEliderTest, MAYBE_TestFilenameEliding)
{
    ...
    FileTestcase testcases[] = {
        ...
        { FILE_PATH_LITERAL("c:") + kPathSeparator + FILE_PATH_LITERAL("path") + kPathSeparator + FILE_PATH_LITERAL("filename.pie"),
            "filename.pie" },
        { FILE_PATH_LITERAL("c:") + kPathSeparator + FILE_PATH_LITERAL("path") + kPathSeparator + FILE_PATH_LITERAL("longfilename.pie"),
    ...
    for (size_t i = 0; i < arraysize(testcases); ++i) {
        ...
        EXPECT_EQ(expected, ElideFilename(filepath, font_list, GetStringWidthF(UTF8ToUTF16(testcases[i].output), font_list)));
    }
}
...
TEST(TextEliderTest, MAYBE_ElideTextEllipsis)
{
    ...
    const float kEllipsisWidth = GetStringWidthF(UTF8ToUTF16(kEllipsis), font_list);
    struct TestData {
        const char* input;
        float width;
        const char* output;
...
TEST(TextEliderTest, MAYBE_ElideTextEllipsisFront)
{
    ...
    const std::string kEllipsisStr(kEllipsis);
    const float kEllipsisWidth = GetStringWidthF(UTF8ToUTF16(kEllipsis), font_list);
    const float kEllipsis23Width = GetStringWidthF(UTF8ToUTF16(kEllipsisStr + "23"), font_list);
    struct TestData {
        const char* input;
        float width;
        const base::string16 output;
...
TEST(TextEliderTest, MAYBE_ElideTextLongStrings)
{
    ...
    base::string16 long_string_middle(data_scheme + base::string16(number_of_as - number_of_trailing_as, 'a') + kEllipsisStr + base::string16(number_of_trailing_as, 'a'));
    ...
    for (size_t i = 0; i < arraysize(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].truncated_y ? INSUFFICIENT_SPACE_VERTICAL : 0,
            ElideRectangleText(UTF8ToUTF16(cases[i].input),
                ...
                &lines));
        if (cases[i].output) {
            const std::string result = UTF16ToUTF8(base::JoinString(lines, ASCIIToUTF16("|")));
    ...
    for (size_t i = 0; i < arraysize(cases); ++i) {
        ...
        EXPECT_EQ(cases[i].truncated_x ? INSUFFICIENT_SPACE_HORIZONTAL : 0,
            ElideRectangleText(UTF8ToUTF16(cases[i].input),
                ...
                &lines));
        if (cases[i].output) {
            const std::string result = UTF16ToUTF8(base::JoinString(lines, base::ASCIIToUTF16("|")));
            EXPECT_EQ(cases[i].output, result) << "Case " << i << " failed!";
        } else {
            EXPECT_TRUE(lines.empty()) << "Case " << i << " failed!";
        }
...
TEST(TextEliderTest, MAYBE_ElideRectangleTextLongWords)
{
    ...
    const base::string16 kElidedTesting = UTF8ToUTF16(std::string("Tes") + kEllipsis);
...
TEST(TextEliderTest, MAYBE_ElideRectangleTextCheckLineWidth)
{
    ...
    std::vector<base::string16> lines;
    EXPECT_EQ(0, ElideRectangleText(UTF8ToUTF16(text), font_list, kAvailableWidth, kAvailableHeight, WRAP_LONG_WORDS, &lines));
...
#define WIDTH(x) GetStringWidthF(UTF8ToUTF16(x), font_list)
    EXPECT_EQ(WIDTH("The administrator for this account has"),
        WIDTH("The ") + WIDTH("administrator ") + WIDTH("for ") + WIDTH("this ") + WIDTH("account ") + WIDTH("has"));
#undef WIDTH
